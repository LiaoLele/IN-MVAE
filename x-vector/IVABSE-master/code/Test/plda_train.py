...
from pkg.utils import CollateFnWrapper
from pkg.config.hparam_pldatrain import hparam as hp
from pkg.Dataset.dataset import Dataset
from pkg.Dataset.dataset import TrainBatchSampler
...
def get_embds(egsfile_path, model_path, out_path, device, use_slide=False, script_prefix=None, data_prefix=None, suffix='0', sub_state=0):
    ...
    if sub_state == 0:
        ...
        if os.path.exists(os.path.join(out_path, "xvec_list.train." + suffix + ".pkl")):
            ...
            assert len(pretrained_dict_rename) == len(spkr_model_dict)
            ...
            for i, in_data in enumerate(dataloader):
                ...
                with torch.no_grad():
                    ...
                    vec = spkr_model.extract_embd(data, use_slide=use_slide)  # use_slide=False because sample lengths in egs.train.pkl are all in the range of [160, 200]
...
def get_lda_transform(xvec_path, out_path, lda_dim=128, centerization=True, suffix='0', sub_state=0):
...
def main(state=0, sub_state=0, script_prefix=None, data_prefix=None):
    if state == 0:
        """ extract embeddings from training data
        and calculate mean of all embeddings """
        traindata_path = os.path.join(script_prefix, hp.path.traindata_path)
        model_path = os.path.join(script_prefix, hp.path.xvecmodel_path)
        out_path = os.path.join(os.path.dirname(os.path.dirname(model_path)), 'PLDA')
        device = torch.device(hp.device)
        suffix = 'augdata'
        get_embds(traindata_path, model_path, out_path, device, use_slide=False,
                  script_prefix=script_prefix, data_prefix=data_prefix, sub_state=sub_state, suffix=suffix)

    elif state == 1:
        """ get LDA transformation matrix """
        model_path = os.path.join(script_prefix, hp.path.xvecmodel_path)
        xvec_path = os.path.join(os.path.dirname(os.path.dirname(model_path)), 'PLDA', 'xvec_ndarray.zero_mean.train.augdata.pkl')
        out_path = os.path.join(os.path.dirname(os.path.dirname(model_path)), 'PLDA')
        lda_dim = hp.data.lda_dim
        suffix = '0'
        get_lda_transform(xvec_path, out_path, lda_dim=lda_dim, centerization=True, suffix='0', sub_state=sub_state)

    elif state == 2:
        """ get PLDAbase and whitening matrix """
        suffix = '0'
        model_path = os.path.join(script_prefix, hp.path.xvecmodel_path)
        model_path = os.path.join(os.path.dirname(os.path.dirname(model_path)), 'PLDA')
        lda_path = os.path.join(model_path, 'lda_machine.withdimreduction.0.hdf5')
        xvec_path = os.path.join(model_path, 'xvec_ndarray.zero_mean.train.augdata.pkl')
        out_path = model_path
        iter_num = 15
        get_plda_machine(lda_path, xvec_path, out_path, iter_num=iter_num, centerization=True, length_normalization=True, suffix=suffix, sub_state=sub_state)


if __name__ == "__main__":
    main(state=2, sub_state=0, script_prefix=hp.path.script_prefix, data_prefix=hp.path.data_prefix)