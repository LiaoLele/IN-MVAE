...
class VariantTanh(nn.Module):
    def __init__(self, alpha):
        super().__init__()
        self.alpha = alpha
    
    def forward(self, x):
        exp_x = (self.alpha * x).exp()
        exp__x = (-self.alpha * x).exp()
        y = (exp_x - exp__x) / (exp_x + exp__x)
        return y
...
def conv_banks_forward(x, conv_bank_layers, activation_layer):
    '''The ConvBank layer is used in both the speaker encoder and content encoder
    to better capture long-term information'''
    out = []
...
class SpeakerEncoder(nn.Module):
    ...
    def __init__(self, c_in, c_out, n_conv_blocks, n_dense_blocks,
        ...
        # avg_pool1d enforces the speaker encoder to learn global information only
    ...
    def forward(self, x):
        ...
        return out
        # return spkr_embd (batch_size, c_out)
...
class ContentEncoder(nn.Module):
    ...
    def __init__(self, c_in, c_out, n_conv_blocks,
        ...
        # self.bottle_neck_layer = nn.Conv1d(c_in_next, self.c_h[0], 1)
        self.convblock_first_layers = nn.ModuleList(
    ...
    def forward(self, x):
        # x = torch.log(x + EPS)
        # out = conv_banks_forward(x, self.conv_bank_layers, self.act_layer)
        # out = self.bottle_neck_layer(pad_input(out, self.bottle_neck_layer.kernel_size[0]))
...
class Decoder(nn.Module):
    ...
    def __init__(self, c_cont, c_spkr, c_out, n_conv_blocks, 
        ...
        self.input_layer = nn.Conv1d(c_cont, c_h[0], 1) 
        self.convblock_first_layers = nn.ModuleList(
            [nn.Conv1d(c_h[i], c_h[i + 1], kernel_size) for i in range(self.n_conv_blocks)]
        )
        self.convblock_second_layers = nn.ModuleList(
    ...
    def pixelshuffle(self, x, upscale=2):
        '''PixelShuffle1d layer is used in the decoder for upsampling.
        '''
...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x_spectrogram):
        ...
        cont_embd = content_log_var.new(*content_log_var.size()).normal_(0, 1)
        # cont_embd = content_log_var.new_empty(content_log_var.size()).normal_(0, 1)
        cont_embd = cont_embd * torch.exp(content_log_var / 2) + content_mu
        # x_log_var = self.decoder(cont_embd, spkr_embd) + self.log_factor
        x_log_var = self.decoder(cont_embd, spkr_embd)
    ...
    #     '''根据输入的信号的频谱来推断spkr_embd，把输入的信号裁成200frame每段，很多段合在一起作为batch'''
    ...
    def get_speaker_embeddings(self, target_feat, target_seglen=200, target_seghop=0.5):
        if target_feat.dim() == 2:
            ...
            raise ValueError("Input without the correct dimension!")
        spkr_embd = self.speaker_encoder(target_feat)
        return spkr_embd
...
class Loss():
    ...
    def basic(self):
        L_rec = (self.oracle.pow(2).log() - self.x_log_var.pow(2).log()).exp() + 2*self.x_log_var.log()
        L_rec = self.weighted_loss(L_rec)
        return L_rec

    def l1_loss(self):
        # L_rec = F.l1_loss(self.x_log_var, (self.oracle + EPS).log(), reduction='none')
        L_rec = F.l1_loss(self.x_log_var, self.oracle, reduction='none')
        # When reduction is 'none', returns a loss per batch element and ignores size_average
        L_rec = self.weighted_loss(L_rec)
        return L_rec

    def mse_loss(self):
        # L_rec = F.mse_loss(self.x_log_var, (self.oracle + EPS).log(), reduction='none')
        L_rec = F.mse_loss(self.x_log_var, self.oracle, reduction='none')
        L_rec = self.weighted_loss(L_rec)
        return L_rec

    def kl_loss(self):
        L_kl = 0.5 * torch.sum(self.content_mu.pow(2) + self.content_log_var.exp() -1 - self.content_log_var)
    ...
    def is_diff(self):
        ...
        is_div = torch.sum(self.oracle / (self.x_log_var + EPS) - ((self.oracle + EPS).log() - self.x_log_var.log()) - 1, axis=(1, 2))
