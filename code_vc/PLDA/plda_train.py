...
def get_embds(egsfile_path, model_path, out_path, device, use_slide=False, data_prefix=None, suffix='0', sub_state=0):
...
def main(state=0, sub_state=0, data_prefix=None):
    if state == 0:
        ...
        model_path = hp.path.xvecmodel_path
        out_path = hp.path.out_path
        ...
        model_path = hp.path.xvecmodel_path
        xvec_path = os.path.join(hp.path.out_path, 'xvec_ndarray.zero_mean.train.0.pkl')
        out_path = hp.path.out_path
        ...
        # suffix = '0'
        lda_path = os.path.join(hp.path.out_path, 'lda_machine.withdimreduction.0.hdf5')
        xvec_path = os.path.join(hp.path.out_path, 'xvec_ndarray.zero_mean.train.0.pkl')
        out_path = hp.path.out_path
        iter_num = 15
