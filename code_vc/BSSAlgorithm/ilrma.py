...
def ilrma(mix, n_iter, n_basis=2, convg_tol=1e-5, return_matrix=False,
...
def ilrma_mypra(mix, n_iter, n_basis=2, convg_tol=1e-6, return_matrix=False,
                          alternate_align_init=False):
    """Implementation of ILRMA (Independent Low-Rank Matrix Analysis).
    This algorithm is called ILRMA1 in http://d-kitamura.net/pdf/misc/AlgorithmsForIndependentLowRankMatrixAnalysis.pdf
    It only works in determined case (n_sources == n_channels).
    ...
    if W0 is None:
        ...
        if alternate_align_init:
            for i in range(1, n_freq, 2):
                W[i, :, :] = np.array([[0, 1], [1, 0]], dtype=np.complex64)
    else:
        W = W0.copy()
    ...
    for t in range(n_iter):
        ...
        # convergence criterion
        Obj = np.sum(P / R + np.log(R))\
            - 2 * n_frames * np.sum(np.log(np.abs(np.linalg.det(W))))
        if np.isnan(Obj):
            flag = True
        dObj = abs(Obj - pObj) / abs(Obj)
        pObj = Obj
