...
def MakePair(srcinfo_path, out_dir, src_num=2, mode='print_avaliable_pair_number',
    ...
    if len(list(spk2utt.keys())) == 1:
        ...
        subset_name = [list(spk2utt.keys())[0], list(spk2utt.keys())[0]]
        subset_data = list(spk2utt.values())[0]
        src_num = 2
...
def main(state=0, sub_state=0, ssd_prefix=None, hdd_prefix=None):
    ...
    if state == 1:
        ...
        pair_path = os.path.join(hdd_prefix, 'DATASET/LibriSpeech/inference/Pair.clean.100spkr-max.test.2src-FF-40_1.xls')
        assign_key = '0.360'
        mode = 1
    ...
    if state == 2:
        sir_range = [-5, 0, 5]
        pair_path = os.path.join(hdd_prefix, 'DATASET/LibriSpeech/inference/Pair.clean.100spkr-max.test.2src-FF-40_1.t60-36ms.xls')
        usecols = ['RelativePath', 'Offset', 'Duration', 'Sr', 'Subset']
        use_random = False
