...
def MakePair(srcinfo_path, out_dir, src_num=2, mode='print_avaliable_pair_number',
    ...
    if len(list(spk2utt.keys())) == 1:
        ...
        subset_name = list(spk2utt.keys())
        subset_data = list(spk2utt.values())
        src_num = len(subset_name)
    ...
    pair_list = []
    subset_data_keylist = [list(data.keys()) for data in subset_data]
    subset_data_keynum = [len(keylist) for keylist in subset_data_keylist]
    pair_pool = list(product(*subset_data_keylist))
    random.shuffle(pair_pool)
    pair_pool = iter(pair_pool)
    for keylist in subset_data_keylist:
        random.shuffle(keylist)
        if len(keylist) < max(subset_data_keynum):
            pad_num = max(subset_data_keynum) - len(keylist)
            keylist.extend(random.choices(keylist, k=pad_num))
    pair_list = list(zip(*subset_data_keylist))
    if len(pair_list) >= n_pair:
        pair_list = pair_list[: n_pair]
    else:
        pair_set = set(pair_list)
        while len(pair_list) < n_pair:
            new = next(pair_pool)
            if new not in pair_set:
                pair_list.append(new)
                pair_set.add(new)

    # choose utterance
    pair2utts, pair2totaluttnum = get_pair2utts(pair_list, subset_data)
    ...
    for spkrs, uttids in pair2utts.items():
        ...
        for uttid in chosen_uttids:
            for i in range(src_num):
                mix2pair[i].append(list(subset_data[i][spkrs[i]][uttid[i]]) + [fs] + [subset_name[i]])
...
def main(state=0, sub_state=0, ssd_prefix=None, hdd_prefix=None):
    if state == 0:
        ...
        # srcinfo_path = os.path.join(hdd_prefix, 'DATASET/LibriSpeech/test/ampnorm_meannorm_trim30/spk2utt.clean.40spkr-max.pkl')
    ...
    if state == 1:
        ...
        pair_path = os.path.join(hdd_prefix, 'DATASET/LibriSpeech/inference/Pair.clean.100spkr-max.test.2src-FM-40_1.xls')
        # pair_path = os.path.join(hdd_prefix, 'DATASET/LibriSpeech/inference/Pair.clean.40spkr-max.2src-FM-40_1.xls')
        assign_key = '0.360'
        mode = 1
    ...
    if state == 2:
        sir_range = [-5, 0, 5]
        pair_path = os.path.join(hdd_prefix, 'DATASET/LibriSpeech/inference/Pair.clean.100spkr-max.test.2src-FM-40_1.t60-36ms.xls')
        # pair_path = os.path.join(hdd_prefix, 'DATASET/LibriSpeech/inference/Pair.clean.40spkr-max.2src-FM-40_1.t60-36ms.xls')
        usecols = ['RelativePath', 'Offset', 'Duration', 'Sr', 'Subset']
        use_random = False
